...
;--------------------------------------------------------
	.area _CODE
;basic-font-words.c:46: void draw_basic_font_word(
;	---------------------------------
...
_draw_basic_font_word::
	dec	sp
	dec	sp
;basic-font-words.c:55: for (uint8_t i = 0; i < character_count; ++i)
	ld	e, #0x00
00108$:
	ld	a, e
	ldhl	sp,	#6
	sub	a, (hl)
	jr	NC, 00109$
;basic-font-words.c:57: uint8_t character = characters[i];
	dec	hl
	push	de
	ld	d, #0x00
	ld	a, (hl-)
	...
	ld	b, h
	ld	a, (bc)
;basic-font-words.c:59: if (character == BASIC_FONT_SKIP_CHARACTER)
	ld	c, a
	inc	a
	jr	Z, 00103$
;basic-font-words.c:64: set_sprite_tile(tile_index_start + i, character);
	ldhl	sp,	#7
	ld	a, (hl)
	add	a, e
	ld	d, a
...
;	spillPairReg hl
	...
	inc	hl
	ld	(hl), c
;basic-font-words.c:68: start_position_y + (i * position_step_y));
	push	de
	ldhl	sp,	#13
	ld	d, (hl)
	push	de
	...
	ldhl	sp,	#0
	ld	(hl), a
;basic-font-words.c:67: start_position_x + (i * position_step_x),
	push	de
	ldhl	sp,	#12
	ld	d, (hl)
	push	de
